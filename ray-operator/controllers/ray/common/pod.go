...
func DefaultWorkerPodTemplate(ctx context.Context, instance rayv1.RayCluster, workerSpec rayv1.WorkerGroupSpec, podName string, fqdnRayIP string, headPort string, replicaGrpName string, numHostIndex int) corev1.PodTemplateSpec {
	...
	if multihostIndexingEnabled {
		podTemplate.Labels = addMultihostIndexingPodLabels(instance.Name, workerSpec.GroupName, podTemplate.Labels, replicaGrpName, numHostIndex)
	}
...
func addMultihostIndexingPodLabels(rayClusterName string, groupName string, currentLabels map[string]string, replicaGrpName string, numHostIndex int) map[string]string {
